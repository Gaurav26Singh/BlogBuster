...
app.get('/:type/profile/:uname',isLoggedin,async(req,res)=>{
    // console.log(req.user);
    let user = await userSchema.findOne({username:req.params.uname}).populate("posts");
...
app.post('/edit/:pid', isLoggedin,async(req,res)=>{
    ...
    let post = await postSchema.findOneAndUpdate({_id:req.params.pid},{content:req.body.content,date:Date.now(),edited:true},{new:true});
    let user = await userSchema.findOne({email:req.user.email});
    // console.log(post);
    res.redirect('/profile');
...
app.post('/search', isLoggedin,async(req,res)=>{
    ...
    let posts = await postSchema.find({$text:{$search:req.body.search}}).populate("user").sort({date:-1});
    let visitor = await userSchema.findOne({email:req.user.email});
    msg = req.body.search;
    if(posts.length == 0){
        res.render("noresult");
...
app.get('/search', isLoggedin,async(req,res)=>{
    ...
    if(dv=='nf' && lv=='ml')
        {
             let posts = await postSchema.find({$text:{$search:msg}}).populate("user").sort({size:-1,date:-1});
             res.render('sresult',{posts,visitor,msg,moment,dv,lv});
        }
    else if(dv=='nf' && lv=='ll'){
        let posts = await postSchema.find({$text:{$search:msg}}).populate("user").sort({size:1,date:-1});
        ...
        let posts = await postSchema.find({$text:{$search:msg}}).populate("user").sort({date:1});
        res.render('sresult',{posts,visitor,msg,moment,dv,lv});
    }
    else{
        let posts = await postSchema.find({$text:{$search:msg}}).populate("user").sort({date:-1});
...
app.post('/apply',isLoggedin,async(req,res)=>{
    ...
    if(dv=='nf' && lv=='ml')
        {
             let posts = await postSchema.find({}).populate("user").sort({size:-1,date:-1});
            res.render('feed',{posts,visitor,moment,dv,lv});
        }
    else if(dv=='nf' && lv=='ll'){
        let posts =  await postSchema.find({}).populate("user").sort({size:1,date:-1});
